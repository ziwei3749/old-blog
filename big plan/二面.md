...
```
 ...
 - Vuex
  ...
  移动端页面适配有很多需要注意的细节。那暂时抛开细节,布局适配方面主要面对的问题就是每一个手机尺寸和分辨率dpr不同。
  
  - 1.首先我们会选择比较常见的手机的宽高,作为基准,比如iphone6(375 * 667)
  - 2.对于iphon6的retina屏幕,它的dpr是2,所以实际设计稿也会做成750 * 1334。
  ...
  - 还需要设置meta标签,让这个viewport的widht=device-width inital-sale=1.0 user-scalable=0. 如果你不设置的话会出现横向滚动条
  
  ```
  
  
 > 2.细节问题: 比如retina下的位图图片如何处理, border:1px如何处理,字体大小问题
